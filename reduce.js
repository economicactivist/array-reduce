...
function extractValue(arr, key) {
  const extractedValueArray = arr.reduce((obj, nextObj) => {
...
function vowelCount(str) {
    ...
    let vowels = 'aeiou'
    const countedVowels =splitStr.reduce((acc, nextLetter)=>{
...
*/
//needed help from the solution, not very intuitive
function addKeyAndValue(arr, key, value) {
    const keyAndValueAdded = arr.reduce((obj, nextObj, idx)=>{
        obj[idx][key] = value
        return obj
    }, arr)
...
function partition(arr, callback) {
    ...
    let b;
    let c = []
    a =  arr.filter(e=>callback(e))
    
    b =  arr.filter(e=>!callback(e))
    c.push(a,b)
    return c
